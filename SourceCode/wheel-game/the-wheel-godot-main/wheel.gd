...
@export var wheel_size:Vector2 = Vector2(200,200): ## a vector2 that specifies the wheel's custom minimum size.
	set(value):
		wheel_size = value
...
#@onready var level_list:Array[Control] = [%Level1, %Level2]; 
...
var slice_position:Array[Control] = [%slice1, %slice2, %slice3, %slice4];  ## base score values for the slices
...
const Slice_pos:Array[Array] = [[180,90,0,270], [270,0,180,90], [180,90,0,270], [0,180,90,270], [180,90,0,270], [180,90,0,270]]; 
...
func _process(_delta): 
	#print(current_level)    
	#print(%Goal.if_entered);    
	if (%Reset.reset_clicked):  
		print("test");  
		movement = Vector2.ZERO; 
		reset(); 
	if (%Goal.if_entered && !(current_level > level_list.size() - 1)): 
		current_level += 1;   
		set_stage(); 
		reset(); 
...
func set_stage():    
	movement = Vector2.ZERO;  
	level_list[current_level - 1].hide() 
	level_list[current_level - 1].collision_layer = 8;#set_deferred("disabled", true);
	#level_list[current_level - 1].set_process(false)
	#level_list[current_level - 1].set_physics_process(false) 
	level_list[current_level].show() 
	level_list[current_level].collision_layer = 1;#set_deferred("disabled", true);
	#level_list[current_level].set_process(true)
	#level_list[current_level].set_physics_process(true)

func _physics_process(_delta): 
	#position += movement * delta;  
	
	#print(Goal.level); 
	#print(Goal.has_method("get_level"));
	#print(Goal.get_level()); 
		...
		if int(round(x.rotation_degrees)) == direction: 
			...
			for i in range(4): 
				if DIRECTIONS[i] == direction:     
					print(slice_position[i]);  
					#print(%slice1); 
					if slice_position[i] == %slice1: 
						movement = check_direction(direction, 4000);  
					elif slice_position[i] == %slice2: 
						movement = check_direction(direction, 6000);  
					elif slice_position[i] == %slice3: 
						movement = check_direction(direction, 8000);  
					elif slice_position[i] == %slice4:  
						movement = check_direction(direction, 10000); 
						
	
			rotate_slices()  
			slice_position = rotate_control_array(slice_position);  
			#print(base_numbers); 
func check_direction(slice:float, move:int):  
	if (slice == 90.0): 
		return Vector2(move, 0); 
	elif (slice == 270.0):  
		return Vector2(-1*(move), 0);  
	elif (slice == 0.0):
		return Vector2(0, -1*(move)); 
	elif (slice == 180.0):
		return Vector2(0, move);
...
func rotate_slices()->void:
	...
	#for i in range(4): 
	#	slices[i].rotation_degrees = current_value_mappings[i]  # sets the slice rotations to our value mappings  
...
func reset()->void:
	...
	for x:int in DIRECTIONS.size(): # assigns the slice value to the direction of the corresponding slice
		for j:int in current_value_mappings.size():
			slices[j].rotation_degrees = Slice_pos[current_level][j];  # sets the slice rotations to our value mappings  
			if DIRECTIONS[x] == current_value_mappings[j]:
				slice_values[x] = x+1
	for i in range(4): 
		if (slices[i].rotation_degrees == 0): 
			slice_position[0] = slices[i]; 
		elif (slices[i].rotation_degrees == 90): 
			slice_position[1] = slices[i]; 
		elif (slices[i].rotation_degrees == 180): 
			slice_position[2] = slices[i]; 
		elif (slices[i].rotation_degrees == 270): 
			slice_position[3] = slices[i];
...
func rotate_control_array(arr:Array[Control]): 
	var temp_arr:Array[Control] = [];    
